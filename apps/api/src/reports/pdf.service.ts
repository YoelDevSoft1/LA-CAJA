...
export class PdfService {
  ...
  async generateSalesByDayPDF(
    ...
    const report = await this.reportsService.getSalesByDay(storeId, startDate, endDate);
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const [method, data] of Object.entries(report.by_payment_method)) {
        doc.text(`${method}: ${data.count} ventas - BS: ${data.amount_bs.toFixed(2)} - USD: ${data.amount_usd.toFixed(2)}`);
      }
      doc.moveDown();
      ...
      doc.fontSize(10);
      doc.text('Fecha | Ventas | Total BS | Total USD | Ganancia BS | Ganancia USD', { underline: true });
  ...
  async generateShiftsPDF(
    ...
    const report = await this.reportsService.getShiftsReport(storeId, startDate, endDate, cashierId);
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.text(`Total Diferencias BS: ${report.total_differences_bs.toFixed(2)}`);
      doc.text(`Total Diferencias USD: ${report.total_differences_usd.toFixed(2)}`);
      doc.moveDown();
      ...
      for (const cashier of report.by_cashier) {
        doc.text(`${cashier.cashier_name}: ${cashier.shifts_count} turnos - Ventas BS: ${cashier.total_sales_bs.toFixed(2)} - Diferencias BS: ${cashier.total_differences_bs.toFixed(2)}`);
      }
      doc.moveDown();
      ...
      doc.fontSize(10);
      for (const shift of report.shifts.slice(0, 50)) { // Limitar a 50 turnos
        doc.text(
  ...
  async generateArqueosPDF(
    ...
    const report = await this.reportsService.getArqueosReport(storeId, startDate, endDate);
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.fontSize(12);
      for (const cashier of report.by_cashier) {
        doc.text(`${cashier.cashier_name}: ${cashier.arqueos_count} arqueos - Diferencias BS: ${cashier.total_differences_bs.toFixed(2)}`);
      }
      doc.moveDown();
      ...
      doc.fontSize(10);
      for (const arqueo of report.arqueos.slice(0, 50)) { // Limitar a 50 arqueos
        doc.text(
  ...
  async generateExpiringProductsPDF(
    ...
    const report = await this.reportsService.getExpiringProductsReport(storeId, daysAhead);
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const product of report.by_product) {
        doc.text(`${product.product_name}: ${product.lots_count} lotes - ${product.total_quantity} unidades`);
        for (const exp of product.expiration_dates) {
          doc.fontSize(10).text(`  - Lote ${exp.lot_number}: ${exp.quantity} unidades - Vence: ${exp.expiration_date.toLocaleDateString('es-VE')} (${exp.days_until_expiration} días)`, { indent: 20 });
        }
        doc.moveDown(0.5);
      }
