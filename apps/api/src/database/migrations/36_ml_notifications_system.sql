...
CREATE TABLE IF NOT EXISTS notification_templates (
  ...
  template_key VARCHAR(100) NOT NULL, -- e.g., 'demand_high', 'stock_alert', 'ml_insight'
  name VARCHAR(255) NOT NULL,
  description TEXT,

  -- Template content (multi-language support)
  content JSONB NOT NULL, -- { "es": { "title": "...", "body": "..." }, "en": { ... } }

  -- Template variables schema
  variables_schema JSONB, -- Define expected variables and types

  -- Template type and category
  template_type VARCHAR(50) NOT NULL, -- 'ml_insight', 'alert', 'recommendation', 'digest'
  category VARCHAR(50) NOT NULL, -- From existing notification categories

  -- ML-specific configuration
  ml_trigger_config JSONB, -- { "confidence_threshold": 80, "severity": "high" }

  -- Channel-specific templates
  email_template TEXT, -- Handlebars HTML template for email
  push_template TEXT,  -- Template for push notifications
  in_app_template TEXT, -- Template for in-app notifications

  -- Priority and scheduling
  default_priority VARCHAR(20) DEFAULT 'medium', -- low, medium, high, urgent
  default_channels TEXT[] DEFAULT ARRAY['in_app'], -- Default delivery channels
...
CREATE TABLE IF NOT EXISTS ml_insights (
  ...
  insight_type VARCHAR(50) NOT NULL, -- 'demand_forecast', 'anomaly', 'recommendation', 'trend'
  insight_category VARCHAR(50) NOT NULL, -- 'product', 'sales', 'inventory', 'revenue'

  -- Entity linking
  entity_type VARCHAR(50), -- 'product', 'sale', 'category'
  entity_id UUID,

  -- ML model information
  model_type VARCHAR(50) NOT NULL, -- 'demand_prediction', 'anomaly_detection', 'recommendation'
  ...
  -- ML-specific data
  ml_data JSONB NOT NULL, -- { "predicted_value": 150, "current_value": 75, "trend": "up" }
  ...
  suggested_actions JSONB, -- [{ "label": "Increase Stock", "action": "reorder", "params": {...} }]
...
CREATE TABLE IF NOT EXISTS notification_analytics (
  ...
  delivery_channel VARCHAR(50), -- 'email', 'push', 'in_app', 'websocket'
  ...
  time_to_action_seconds INTEGER, -- Time from delivery to action
...
CREATE TABLE IF NOT EXISTS notification_rate_limits (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  store_id UUID NOT NULL REFERENCES stores(id) ON DELETE CASCADE,
  user_id UUID REFERENCES users(id) ON DELETE CASCADE, -- NULL for store-wide limits

  -- Limit configuration
  category VARCHAR(50), -- NULL for all categories
  ...
  -- Priority override
  allow_critical_override BOOLEAN DEFAULT true, -- Allow critical notifications to bypass limits
...
CREATE TABLE IF NOT EXISTS ml_notification_rules (
  ...
  ml_insight_type VARCHAR(50) NOT NULL, -- 'demand_forecast', 'anomaly', etc.
  trigger_conditions JSONB NOT NULL, -- { "confidence_min": 80, "severity": ["high", "critical"] }

  -- Notification template
  template_id UUID REFERENCES notification_templates(id),
  template_key VARCHAR(100), -- Fallback if template_id not set

  -- Target audience
  target_roles TEXT[], -- ['store_manager', 'inventory_manager']
  ...
  -- Scheduling
  schedule_cron VARCHAR(100), -- Optional cron expression for scheduled checks
...
-- Add ML-specific columns to existing notifications table
ALTER TABLE notifications
  ...
  ADD COLUMN IF NOT EXISTS engagement_score NUMERIC(5,2) DEFAULT 0, -- Calculated engagement score
...
-- Insert default templates for common ML insights
